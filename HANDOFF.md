...
last_updated: 2026-04-23 (après R22 → R22a → R22b → R22c → R22d → R22e → R22f → R22g+h)
---
...
### Ce qui tourne en prod (2026-04-23, après R22b → R22c → R22d → R22e poussés ; R22f+g+h en attente de push)
...
- **Label header** : `R22g · <short_sha>` (bump R22a → R22g en R22g+h, `SYSTEM_VERSION_LABEL` ligne 28 de `site_export.py`).
- **Cron** : 04:00 Europe/Paris (R22f, 2026-04-23). Le workflow `.github/workflows/daily.yml` est planifié à `0 2 * * *` UTC — alignement heure d'été par défaut (= 04:00 CEST, 03:00 CET en hiver). Précédemment 06:00 UTC.
...
1. **Pousser R22f+g+h** — 3 commits locaux en attente (`40724d9` cron 04h, `d6936ca` questions titres legacy + fenêtre 90j stricte, `1fbe4b7` titre site majuscule I). Script `push_R22gh.command` prêt. Après push, vérifier que la page `/items/questions/` ne montre plus de titres "PAxxx (GROUPE) : …" au-delà de la 5e ligne et que l'item 17-11612QE (publié 2025-12-09, ~135j) a bien disparu.
2. **Monter la vague 1 de l'audit — filet de sécurité.** Les trois briques : (a) monitoring pipeline (sources actives / en erreur / à 0 items + écart vs J-7) en fin de digest ; (b) suite de tests « incidents » rétroactive sur les rustines R13 → R22h (un test par régression pour fermer la porte) ; (c) contrat de source auto-généré (`tests/contracts/test_source_<id>.py` : ≥ 1 item / 30 j). Détail §5 vague 1 de `docs/AUDIT_R19.md`.
